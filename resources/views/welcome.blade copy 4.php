    ...
    <div class="row g-4">
      ...
      <div class="col-lg-3 col-md-6">
        <div class="card product-card h-100">
          ...
          <div class="card-body">
            {{-- <h5 class="product-title">Kemeja Casual Premium</h5> --}}
             <h5 class="product-title"><a href="{{ route('welcome.show', $row->id) }}" title="More Details">{{ $row->title }}</a></h5>
              <p>{{ $row->subtitle }}</p>
            ...
            {{-- <p class="card-text text-muted">Kemeja premium berbahan lembut dan nyaman untuk aktivitas harian.</p> --}}
            <div class="d-flex justify-content-between align-items-center mt-3">
              <p class="product-price mb-0">Rp{{ $row->price }}</p>
              {{-- <p class="text-decoration-line-through text-muted fs-6">Rp320.000</p> --}}
            </div>
          </div>
...
<!-- Modal -->
  <div id="buyNowModal" class="modal">
    <div class="modal-content">
      ...
      <h4><sup>Rp</sup><del id="modal-image"></del><span>one-time</span></h4>
      <h4><sup>Rp</sup><span id="modal-price"></span><span>one-time</span></h4>
      <ul id="modal-description"></ul>
      <div class="text-center">
        <a href="#" id="whatsapp-link" class="buy-btn" target="_blank">Order via WhatsApp</a>
      </div>
    </div>
  </div>
...
<a href="#" class="btn btn-success position-fixed bottom-0 start-0 m-4 rounded-circle" style="width: 50px; height: 50px; display: flex; align-items: center; justify-content: center;">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 rounded-4 overflow-hidden">
      <div class="modal-header border-0 bg-primary text-white">
        <h5 class="modal-title fw-bold" id="promoModalLabel">Promo Spesial!</h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body text-center p-4">
        <div class="mb-4">
          <span class="badge bg-warning px-3 py-2 fs-6 mb-3">Penawaran Terbatas!</span>
          <h3 class="text-gradient">Diskon 25% untuk Semua Produk</h3>
          <p class="text-muted">Gunakan kode promo berikut saat checkout:</p>
          <div class="bg-light p-3 rounded-3 mb-3">
            <h4 class="mb-0">ELEGANDISKON25</h4>
          </div>
          <p class="small text-muted">*Berlaku hingga 30 April 2025</p>
        </div>
        <button type="button" class="btn btn-primary btn-lg px-4" data-bs-dismiss="modal">Belanja Sekarang</button>
      </div>
    </div>
  </div>
</div> --}}

<!-- Script untuk show modal after 5 seconds -->
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    window.onscroll = function() {
      var backToTopButton = document.querySelector('.btn-primary.position-fixed');
...
<script>
  ...
  document.querySelectorAll(".buy-btn").forEach(btn => {
    btn.addEventListener("click", function(e) {
      ...
      const card = this.closest(".pricing-item");
      const title = card.querySelector("h3").innerText;
      const subtitle = card.querySelector("p.text-center").innerText;
      const oldPrice = card.querySelector("del").innerText;
      const newPrice = card.querySelector("h4:nth-of-type(2)").innerText.replace("one-time", "").trim();
      const descriptions = Array.from(card.querySelectorAll("ul li span")).map(li => li.innerText).join("\n- ");

      // Isi modal dengan data
      document.getElementById("modal-title").innerText = title;
      document.getElementById("modal-subtitle").innerText = subtitle;
      document.getElementById("modal-image").innerText = image;
      document.getElementById("modal-price").innerText = price;
      document.getElementById("modal-description").innerHTML = descriptions.split("\n").map(d => `<li>${d}</li>`).join("");

      // Buat link WhatsApp
      // const phoneNumber = "+6281228077252";
      const message = `Halo, saya tertarik dengan paket *${title}*:\n\n📌 ${subtitle}\n💰 Harga Lama: Rp ${image}\n💰 Harga Baru: Rp ${price}\n\n🔹Fitur:\n- ${descriptions}\n\nBagaimana cara pembeliannya?`;
      const phoneNumber = "+6282284243138"; // Ganti dengan nomor WA tujuan
      const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;
      document.getElementById("whatsapp-link").setAttribute("href", whatsappUrl);

      modal.style.display = "block";

      // Langsung arahkan ke WhatsApp saat tombol diklik
